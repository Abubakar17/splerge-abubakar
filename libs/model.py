...
class Split_RPN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        top_result= top_result.view(batch_size, 18, height, 1).repeat(1, 1, 1, top_result.shape[3])
        
        #BOTTOM BRANCH
        bot_result= self.conv1x1_bot(output) #convolution layer
        bot_result = torch.mean(bot_result, dim=3)
        bot_result = bot_result.view(batch_size, 1, height, 1).repeat(1, 1, 1, bot_result.shape[3])         
